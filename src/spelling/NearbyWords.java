...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		// inserts a letter in front of, in between, and after each character and checks to see if it is a word.  If so adds to the list.
	...
	public void deletions(String s, List<String> currentList, boolean wordsOnly ) {
		// deletes a character and checks to see if it is a word.  If so it adds to the list.
		for(int index = 0; index < s.length(); index++){
			StringBuffer sb = new StringBuffer(s);
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while(!queue.isEmpty() && retList.size() < numSuggestions){
			...
			for (String wyrd : words){
				if (!visited.contains(wyrd) && retList.size() < numSuggestions){
					visited.add(wyrd);
					queue.add(wyrd);
					if(dict.isWord(wyrd)){
						retList.add(wyrd);
