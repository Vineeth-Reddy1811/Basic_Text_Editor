...
public class WPTree implements WordPath {
	...
	{
	    if(word1 == null || word2 == null){ // check to see if word1 or word2 are null
	    	return null;
	    }
		
		if(!nw.dict.isWord(word2)){ // check to see if word2 is actually a word before doing the search
	    	return null;
	    }
		
		List<WPTreeNode> queue = new LinkedList<WPTreeNode>();  // WPTreeNodes of words to explore
		HashSet<String> visited = new HashSet<String>(); // Set to keep track of what has already been visited
		...
		while(!queue.isEmpty()){
			WPTreeNode curr = ((LinkedList<WPTreeNode>) queue).poll();
			for (String w : nw.distanceOne(curr.getWord(), true)){
